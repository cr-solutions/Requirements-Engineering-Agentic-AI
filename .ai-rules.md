...
## 8. Class Inheritance
* We use explicit parent calls, to prevent any confusion in case of multiple class inheritance.
...
* **Note:** In Kiro, this folder is separate from `.kiro/steering/` (which holds always-on rules). Context history files are session logs, not steering rules.
